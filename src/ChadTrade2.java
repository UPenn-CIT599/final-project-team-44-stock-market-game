	...
	private Portfolio optionsFourAndFive(Portfolio port, int selectedOption, Scanner s) {
		...
		if (selectedOption == 4) {
			...
			if(port == null) {
				...
				fileName = s.next() + ".csv";
				port = this.initialCashDeposit(amount);
				port.updatePortfolio();
	...
	public int getValidInt(Scanner s) {
		...
		while (!validShares || shares <= 0) {
			try {
				shares = s.nextInt();
				if (shares <= 0) {
					System.out.println("Invalid shares. Please input an number greater than 0.");
				}
				else {
					validShares = true;
				}
				...
				validShares = false;
				System.out.println("InputMismatchException. Please input a valid positive number.");
//				shares = s.nextInt();
				s.nextLine();
				// e.printStackTrace();
			}
	...
	public double getValidDouble(Scanner s) {
		double amount = 0;
		...
		while (!validAmount) {
			try {
				amount = s.nextDouble();
				if (amount < 0) {
					System.out.println("Invalid amount. Please input an number greater than or equal to 0.");
				}
				else {
					validAmount = true;
				}
				...
				validAmount = false;
				System.out.println("InputMismatchException. Please input a valid number.");
//				shares = s.nextInt();
				s.nextLine();
				// e.printStackTrace();
			}
