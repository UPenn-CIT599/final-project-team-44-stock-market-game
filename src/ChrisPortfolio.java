...
public class ChrisPortfolio implements FinancialActivity{
	...
	public ChrisPortfolio(HashMap<String, Position> positions) {
	...
	public void buySell(String name, double units) {
		try {
			Position p;
			String stockSymbol = name.toUpperCase();
			//initializes tradeAction to bought. Will be changed to sold later if shares < 0.
			String tradeAction = "bought";
			int absShares = (int) Math.abs(units);
			double price = Double.parseDouble(quote.getField(stockSymbol, "regularMarketPrice\":(.+?),", "chart"));;
			double netMoney = price * units;
			
			//Liquidating a position
			if (portfolio.containsKey(stockSymbol) && units + portfolio.get(stockSymbol).getShares() == 0) {
				portfolio.remove(stockSymbol);
			}
			else {
				//New purchase of a position
				if (!portfolio.containsKey(stockSymbol) && units > 0) {
					p = new Position(stockSymbol, units, price);
				}
				//add or trim an existing position
				else {
					double newShares = portfolio.get(stockSymbol).getShares() + units;
